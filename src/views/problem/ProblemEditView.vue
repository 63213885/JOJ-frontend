  ...
  <div class="problem-create-wrapper">
    <div class="form-container glass-panel">
      <div class="form-header">
        <h2 class="form-title">修改题目</h2>
        <p class="form-subtitle">更新题目数据</p>
      </div>
...
import type { UpdateProblemRequest } from "../../../generated/models/UpdateProblemRequest";
    ...
    onMounted(async () => {
      ...
      try {
        ...
        if (res.code === 0 && res.data) {
          ...
          formData.timeLimit = d.timeLimit || 1000;
          formData.memoryLimit = d.memoryLimit || 256;
          formData.content = d.content || "";
          formData.inputDesc = d.inputDesc || "";
          formData.outputDesc = d.outputDesc || "";
          ...
          if (Array.isArray(parsedSamples) && parsedSamples.length > 0) {
            formData.samples = parsedSamples.map((s: any) => {
              if (typeof s === "string") {
                try {
                  s = JSON.parse(s);
                } catch (e) {
                  console.warn("Parse individual sample failed", e);
                }
              }
              return {
          ...
          if (typeof parsedTags === "string") {
            try {
              ...
              console.warn("Parse tags failed", e);
            }
          }
          formData.tags = Array.isArray(parsedTags) ? parsedTags : [];
          ...
          if (typeof parsedSource === "string") {
            try {
              ...
              console.warn("Parse source failed", e);
            }
          }
          formData.source = Array.isArray(parsedSource) ? parsedSource : [];

          tagsInput.value = formData.tags.join(", ");
          sourceInput.value = formData.source.join(", ");
        } else {
